...
Original file is located at
    https://colab.research.google.com/drive/1vBrp9qRliLVntI-Ahet-uxOG2l9oLzal
"""
...
def terminate_ngrok_sessions(api_key=None):
    if not api_key:
        logger.warning("No ngrok API key provided.")
        return
    try:
...
def list_output_files():
    try:
        ...
        return f"Generated files: {', '.join(files) if files else 'None'}"
    except Exception as e:
...
crop_rotation = {
    "Rice": ["Wheat", "Gram", "Mustard"], "Wheat": ["Rice", "Moong", "Sugarcane"], "Tulsi": ["Marigold", "Mint", "Jasmine"],
    "Mango": ["Guava", "Litchi", "Banana"], "Rose": ["Marigold", "Jasmine", "Tulsi"], "Tomato": ["Potato", "Cauliflower", "Cucumber"],
    "Pea": ["Wheat", "Mustard", "Gram"], "Marigold": ["Rose", "Tulsi", "Jasmine"], "Banana": ["Mango", "Guava", "Litchi"],
    "Mint": ["Tulsi", "Jasmine", "Marigold"], "Apple": ["Guava", "Litchi", "Mango"], "Onion": ["Potato", "Cauliflower", "Tomato"],
    "Neem": ["Peepal", "Mango", "Guava"], "Lotus": ["Rice", "Jasmine", "Marigold"], "Guava": ["Mango", "Litchi", "Banana"],
    "Maize": ["Wheat", "Gram", "Mustard"], "Gram": ["Rice", "Moong", "Sugarcane"], "Mustard": ["Rice", "Pea", "Wheat"],
    "Sugarcane": ["Wheat", "Gram", "Moong"], "Cucumber": ["Tomato", "Cauliflower", "Onion"], "Jowar": ["Bajra", "Ragi", "Maize"],
    "Bajra": ["Jowar", "Ragi", "Maize"], "Ragi": ["Jowar", "Bajra", "Maize"], "Moong": ["Wheat", "Rice", "Sugarcane"],
    "Arhar": ["Rice", "Wheat", "Moong"], "Potato": ["Onion", "Cauliflower", "Tomato"], "Cauliflower": ["Tomato", "Onion", "Potato"],
    "Jasmine": ["Rose", "Tulsi", "Marigold"], "Litchi": ["Mango", "Guava", "Banana"], "Peepal": ["Neem", "Mango", "Guava"]
}

crop_info = {
    "Rice": {"category": "Crop", "info": "Staple food, rich in carbohydrates. Requires 150-200 cm rainfall, 24°C, clayey loamy soil. Kharif crop. Used in biryani, pulao. Uttar Pradesh is India's top rice producer.",
             "nutrients": "N: 80-120 kg/ha, P: 40-60 kg/ha, K: 40-60 kg/ha", "water": "1200-1500 mm", "yield": "4-6 t/ha", "price": "₹20-25/kg"},
    "Wheat": {"category": "Crop", "info": "Used for bread, chapati. Needs 20-100 cm rainfall, 17-20°C, loamy soil. Rabi crop. Uttar Pradesh leads wheat production.",
    ...
    "Sugarcane": {"category": "Crop", "info": "Primary sugar source. Needs 75-150 cm rainfall, 21-27°C, loamy soil. Kharif crop. Uttar Pradesh is India's top producer.",
...
def recommend(city, temp, humidity, rain, soil, season):
    logger.info(f"Inputs: city={city}, temp={temp}, humidity={humidity}, rain={rain}, soil={soil}, season={season}")

    if not all([city, soil, season]):
        return "❌ Please select City, Soil, and Season", None, None, None, None, None, None, None, None, None, None
    if temp is not None and not (0 <= temp <= 50):
        return "❌ Temperature must be 0-50°C", None, None, None, None, None, None, None, None, None, None
    if humidity is not None and not (0 <= humidity <= 100):
        return "❌ Humidity must be 0-100%", None, None, None, None, None, None, None, None, None, None
    if rain is not None and not (rain >= 0):
        return "❌ Rainfall must be >=0 mm", None, None, None, None, None, None, None, None, None, None
    ...
    try:
        ...
        return f"❌ Prediction error: {e}", None, None, None, None, None, None, None, None, None, None
...
try:
    ...
    try:
        ...
        if port is None:
            raise OSError("No available ports in range 7860-7870.")
        ngrok.set_auth_token("YOUR_NGROK_AUTH_TOKEN")
        terminate_ngrok_sessions("YOUR_NGROK_API_KEY")
        public_url = ngrok.connect(port)
        iface.launch(share=False, server_port=port)
        logger.info(f"Gradio interface running at: {public_url}")
    except Exception as e:
        logger.error(f"Error launching Gradio with ngrok: {e}")
        print("Gradio launch failed. Try restarting the runtime or using a different port.")
        raise